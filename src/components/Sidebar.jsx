...
export const Sidebar = ({ mobileOpen = false, onClose = () => {} }) => {
	...
	const content = (
		<div className="glossy relative flex h-full w-64 flex-col overflow-hidden bg-ink text-white">
			<div className="glossy-sheen" />

			<div className="relative flex items-center gap-2 border-b border-white/10 px-5 py-5">
				<div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ backgroundColor: theme.color }}>
					<ScanFace className="h-4 w-4 text-white" strokeWidth={2} />
				</div>
				<span className="font-display text-base font-semibold">Access Portal</span>
				<button
					onClick={onClose}
					className="ml-auto rounded-md p-1 text-white/60 transition-colors hover:bg-white/10 hover:text-white md:hidden"
				>
					<X className="h-4 w-4" />
				</button>
			</div>
			...
			<div className="relative px-4 pt-5">
				<p className="px-2 text-[10px] uppercase tracking-[0.2em] text-white/40">Menu</p>
			</div>
				...
				{items.map((item) => {
					...
					return (
						<Link
							...
							style={active ? { backgroundColor: `${theme.color}33` } : undefined}
						>
							{active && (
								<span
			...
			<div className="relative border-t border-white/10 px-5 py-4">
				<p className="text-[10px] uppercase tracking-[0.2em] text-white/40">{theme.clearanceLine}</p>
			</div>
		</div>
	)
	...
	return (
		<>
			{}
			<aside className="sticky top-0 hidden h-screen shrink-0 md:block">{content}</aside>
